...
{
    class DrawFloatingAnimation : DrawAnimation
    {
        private int screenWidth, screenHeight, posX, posY;
        private DrawElement sourceElement;

        public DrawFloatingAnimation(int posX, int posY, int screenWidth, int screenHeight, DrawElement element)
            : base(posX, posY)
        {
        ...
        */
        ...
        public override void draw(Graphics graphics)
        {
            ...
            if (random.Next(5) == 0)
            {
                posX += (lastX = random.Next(20) - 10);
                posY += (lastY = random.Next(20) - 10);
            }
            else
            {
                posX += lastX;
